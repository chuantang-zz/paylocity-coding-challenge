...
export class InfoCardsComponent implements OnInit, OnDestroy, AfterViewChecked
{
	...
	public addRandomValue(param)
	{
		...
		{
			case 'orders':
				for (let i = 1; i < 30; i++) { this.orders[0].series.push({ 'name': 1980 + i, 'value': Math.ceil(Math.random() * 1000000) }); }

				return this.orders;
			case 'customers':
				for (let i = 1; i < 15; i++) { this.customers[0].series.push({ 'name': 2000 + i, 'value': Math.ceil(Math.random() * 1000000) }); }
